...
import { Plus, Calendar, AlertTriangle, CheckCircle, ArrowLeft, Edit, Trash2 } from 'lucide-react'
...
export default function CustomersPage() {
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          ...
          contract_end_date: newContractEnd,
        })
  ...
  const deleteCustomer = async (customerId: string) => {
    if (!confirm('هل أنت متأكد من حذف هذا العميل؟ سيتم حذف جميع المهام المرتبطة به.')) {
      return
    }
  ...
  const startEdit = (customer: Customer) => {
    ...
    setNewContractEnd(customer.contract_end_date)
  }
                ...
                <Table>
                  ...
                  <TableBody>
                    {customers.map((customer) => {
                      const daysUntilExpiry = getDaysUntilExpiry(customer.contract_end_date)
                      const isExpired = isContractExpired(customer.contract_end_date)
                        ...
                        <TableRow key={customer.id} className={isExpired ? "bg-red-50" : ""}>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              ...
                              {new Date(customer.contract_end_date).toLocaleDateString('ar-SA')}
                            </div>
                          </TableCell>
                          <TableCell>
                            <span
                                ...
                                <Dialog>
                                  ...
                                  <DialogContent>
                                    <DialogHeader>
                                      ...
                                      <DialogDescription>
                                        تحديث بيانات العميل وتاريخ انتهاء العقد
                                      </DialogDescription>
                                    </DialogHeader>
                                    <form onSubmit={updateCustomer} className="space-y-4">
                                      ...
                                      <div className="space-y-2">
                                        <Label htmlFor="editContractEnd">تاريخ انتهاء العقد</Label>
                                        <Input
                                          id="editContractEnd"
                                          type="date"
                                          value={newContractEnd}
