...
import { Plus, CheckCircle, Clock, AlertTriangle, Filter, ArrowLeft, Calendar } from "lucide-react"
...
export default function TasksPage() {
  ...
  const filterTasks = () => {
    ...
    switch (activeFilter) {
      case "today":
        ...
        break
      case "pending-short":
        filtered = tasks.filter((task) => !task.done && task.term === "قصير")
        break
      case "pending-long":
        filtered = tasks.filter((task) => !task.done && task.term === "طويل")
        break
      case "expired-contracts":
        filtered = tasks.filter((task) => new Date(task.customer.contractEnd) < new Date())
        break
      case "completed":
  ...
  const addTask = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!title || !details || !term || !employee || !customerId) return
                      ...
                      {customers.map((customer) => (
                        <SelectItem key={customer.id} value={customer.id.toString()}>
                          <div className="flex items-center justify-between w-full">
                            ...
                            {isContractExpired(customer.contractEnd) && (
              ...
              <Table>
                ...
                <TableBody>
                  {filteredTasks.map((task) => (
                    <TableRow key={task.id} className={isContractExpired(task.customer.contractEnd) ? "bg-red-50" : ""}>
                      <TableCell>
                        <div>
                      ...
                      <TableCell>
                        <div>
                          ...
                          {isContractExpired(task.customer.contractEnd) && (
                      ...
                      <TableCell>
                        <Badge variant={task.term === "قصير" ? "default" : "secondary"}>{task.term} المدى</Badge>
                      </TableCell>
